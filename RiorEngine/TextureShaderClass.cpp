...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"../RiorEngine/TextureVertexShader.hlsl");
	if (error != 0) return false;
	
	// Set the filename of the pixel shader.
	error = wcscpy_s(psFilename, 128, L"../RiorEngine/TexturePixelShader.hlsl");
	if (error != 0) return false;
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	matrixBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER; // Set buffer as constant buffer
...
bool TextureShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	// Map -> A lot quicker than UpdateSubresource. It is used for data that is going to be reloaded each frame or on a very regular basis.
